...
export async function getAllPokemons (req, res) {
    const pokemons = await Pokemon.findAll();
    if (pokemons.length === 0) {  // Vérifier si le tableau est vide
        return res.status(404).json('Aucun Pokemon dans la base');
    }
    res.status(200).json(pokemons);
}
...
export async function getOnePokemon (req, res) {
    ...
    if (!pokemon) {
        return res.status(404).json(`Le Pokemon numéro : ${id} n'existe pas.`);  // Correction du message d'erreur
    }
    res.status(200).json(pokemon);
}
